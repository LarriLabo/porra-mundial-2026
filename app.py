...
.hero::after { content:""; position:absolute; inset:auto -12% -46% -12%; height:120px; background:rgba(255,255,255,.10); transform:rotate(-3deg); }
...
.note-card { background:#fff7db; border:1px solid rgba(242,142,0,.35); border-radius:24px; padding:1rem; color:#383737; font-weight:900; }
@media (max-width: 980px) { .hero-logo-row { grid-template-columns:1fr; } .logo-avatar { display:none; } .metric-grid, .podium-grid, .participant-grid { grid-template-columns:1fr; } .rank-head, .rank-row { grid-template-columns:50px 1.4fr .62fr .62fr .62fr .7fr; font-size:.72rem; } }
</style>
...
def load_data(url):
    ...
    if total_col is None:
        # busca la columna numérica más poblada excluyendo participante
        candidates = [c for c in puntos_tbl.columns if c != p_col]
        best = None
        best_count = -1
        for c in candidates:
            nums = pd.to_numeric(puntos_tbl[c], errors="coerce")
            cnt = nums.notna().sum()
            if cnt > best_count:
    ...
    # Puntos por selección, si existe en la hoja Puntos
    team_points = {}
    try:
...
def simulate_spain_win(ranking, apuestas, level_cols):
    ...
    for _, row in apuestas.iterrows():
        if any(norm(row.get(c, "")) == norm(WINNER) for c in level_cols):
            spain_holders.add(norm(row["PARTICIPANTE"]))
    df = ranking.copy()
...
def assign_prizes(final_df, participants):
    ...
    df["PREMIO"] = 0.0
    first_mask = df["POS"] == 1
    second_mask = df["POS"] == 2
    first_count = int(first_mask.sum())
    second_count = int(second_mask.sum())
    if first_count > 1:
...
def prize_text(v):
    if not v or pd.isna(v) or float(v) == 0:
        return ""
    if abs(float(v) - round(float(v))) < 0.01:
...
def render_podium(final_df):
    ...
    for pos, label, icon in [(1, "1º puesto", "🏆"), (2, "2º puesto", "🥈"), (3, "3º puesto", "🥉")]:
        ...
        if people.empty:
            names_html = "<div class='podium-name'>Sin datos</div>"
            points_html = ""
        else:
            rows = []
            ...
            names_html = "".join(rows)
            points_html = ""
        chunks.append(f"<div class='{cls}'><div class='podium-rank'>{icon}</div><div style='color:#004A5F;font-weight:1000;font-size:1.08rem'>{label}</div>{names_html}{points_html}</div>")
    return "<div class='podium-wrap'><div class='podium-grid'>" + "".join(chunks) + "</div></div>"

def render_final_table(df, limit=None):
    work = df.copy()
    if limit:
    ...
    for _, r in work.iterrows():
        ...
        rows.append(f"""
        <div class='rank-row'>
            <div class='rank-pos'>{int(r['POS'])}º</div>
            <div>{esc(r['PARTICIPANTE'])}</div>
            <div>{int(r['PUNTOS_TOTALES'])}</div>
            <div>{'+' + str(int(r['BONUS_FINAL'])) if int(r['BONUS_FINAL']) else '-'}</div>
            <div class='rank-total'>{int(r['PUNTOS_FINALES'])}</div>
...
def render_selected_teams(apuestas, level_cols, team_points, final_totals):
    ...
    for _, row in work.iterrows():
        ...
        for col in level_cols:
            ...
            pts_html = pts if pts != "" else ""
            picks.append(f"<span class='pick-chip'><span>{esc(team)}</span><b>{pts_html}</b></span>")
        cards.append(
            "<details class='participant-card'>"
            + f"<summary><span class='participant-summary-line'><span>{esc(participant)}</span>{total_html}</span></summary>"
            + f"<div class='pick-grid'>{''.join(picks)}</div>"
            + "</details>"
        )
...
first_count = int((final_df["POS"] == 1).sum())
...
<div class='final-banner'>
  <b>🏁 Se acabó la calculadora.</b> Con la victoria de <b>España</b>, se suman <b>+{CHAMPION_BONUS}</b> puntos a todas las porras que llevaban a España. Ahora sí: este es el cierre final de la clasificación.
</div>
""", unsafe_allow_html=True)
...
<div class='metric-grid'>
  <div class='metric-card'><div class='metric-label'>Participaciones</div><div class='metric-value'>{len(apuestas)}</div></div>
  <div class='metric-card'><div class='metric-label'>Bote total</div><div class='metric-value'>{pot} €</div></div>
  <div class='metric-card'><div class='metric-label'>Porras con España</div><div class='metric-value'>{winner_count}</div></div>
  <div class='metric-card'><div class='metric-label'>Puntos líderes</div><div class='metric-value'>{leader_points}</div></div>
</div>
""", unsafe_allow_html=True)
